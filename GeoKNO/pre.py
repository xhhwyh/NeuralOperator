...
'''
# get a random mask
grid, data_in_mask, data_out_mask = [], [], []
for n in range(ndata):
    mask = np.random.choice([0, 1], data_in_ds.shape[1], p=[0.5, 0.5])
    grid.append(grid_1d[np.nonzero(mask)])
    data_in_mask.append(data_in_ds[n][np.nonzero(mask)])
    data_out_mask.append(data_out_ds[n][np.nonzero(mask)])

features = np.stack((data_in_mask, data_out_mask), axis = 2)
'''
...
# np.savez_compressed("datasets/burgers/geokno_quad_equal_weight_mixedmask_coord_data.npz", nnodes=nodes, node_mask=node_mask, nodes=nodes, node_weights=node_weights, features=features, directed_edges=directed_edges, edge_gradient_weights=edge_gradient_weights)
...
np.savez_compressed("datasets/burgers/geokno_quad_mixedmask_coord_data.npz", nnodes=nodes, node_mask=node_mask, nodes=nodes, node_weights=node_weights, features=features, directed_edges=directed_edges, edge_gradient_weights=edge_gradient_weights)
