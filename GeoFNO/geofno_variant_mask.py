...
class GeoFNO(nn.Module):
    ...
    def forward(self, x, xf):
        ...
        # grid:    (batchsize, ndim, nx)
        # weights: (batchsize, 1   , nx)
        # mask:    (batchsize, 1   , nx)

        bases_c, bases_s, bases_0 = compute_Fourier_bases(grid, self.modes, mask)
        # bases_c: (batchsize, mdoes, nx)
        # bases_s: (batchsize, modes, nx)
        # bases_0: (batchsize, 1    , nx)
        ...
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        # take the indices of points whose mask is one
...
def GeoFNO_train(x_train, y_train, x_test, y_test, mask_size, config, model, save_model_name="./GeoFNO_model"):
    ...
    x_train = torch.zeros((n_train, mask_size, x_train.shape[2])).to(device)
    y_train = torch.zeros((n_train, mask_size, y_train.shape[2])).to(device)
